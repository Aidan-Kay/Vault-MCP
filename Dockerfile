...
# layer on the URL alone, so a new commit on master does NOT invalidate it:
# rebuild with `docker compose build --no-cache vault-index` to pick one up.
RUN git clone --depth 1 https://github.com/Aidan-Kay/vault-index.git .
